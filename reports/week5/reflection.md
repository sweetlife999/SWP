...
- **MVP v2 was more than a release tag; it was a validation point.** Tagging the Sprint 4 increment as MVP v2 meant the team had to prove the product was not only implemented, but verifiable, deployable, and reviewable as a coherent increment. That changed the focus from isolated features to an end-to-end product slice.
...
- **MVP v2 exposed the gap between a working increment and a fully polished product.** The increment was good enough to release and review, but the customer review still surfaced follow-up improvements that belong in the next iteration.
